...
export default function CarInfo(props) {
    ...
    return(
        <>
            <NavLink to ={`/car/${props?.carInfo?.id}`}><Foto photoUrl = {photos[0]?.photoUrl} /></NavLink>
            <NavLink to = {`/car/${props?.carInfo?.id}`} className="nav-link-cars">{`${props?.carInfo?.brand} ${props?.carInfo?.model}`}</NavLink>
        </>
    );
}